...
// Promise.all - takes in an array of promises + shortcircuits if one of the promises is rejected

// Other Promise Combinators: race, allSettled and any

// Promise.race - returns the first settled promise - no matter if fulfilled or rejected, first promise that gets resolved

// (async () => {
//   const res = await Promise.race([
//     getJSON(`https://restcountries.com/v3.1/name/italy`),
//     getJSON(`https://restcountries.com/v3.1/name/austria`),
...
// Promise.allSettled- takes in array of promises, returns array of all Promises resloved or rejected
...
// Promise.any [ES2021] - returns any resolved Promise if there is one inside of the arrayF
...
const loadAll = async arr => {
  const imgs = arr.map(img => await createImage(img));
  // console.log(imgs);
